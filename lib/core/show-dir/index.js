...
const base64include = require("./config.json");

const supportedIcons = styles.icons;
const css = styles.css;

const argv = require('minimist')(process.argv.slice(2), {alias: { tls: 'ssl' }});
      ...
      fs.readdir(dir, (readErr, _files) => {
        ...
        function render(dirs, renderFiles, lolwuts) {
          ...
          const writeRow = (file) => {
            ...
            const ext = file[0].split('.').pop().toLowerCase();
            const _filename = file[0].toLowerCase();

            let classForNonDir = 'default';

            for (let i in base64include)
              if (_filename.slice(-i.length) == i) {
                classForNonDir = base64include[i];
                break;
              }

            let iconClass = `icon-${isDir ? '_blank' : classForNonDir}`;
              
            iconls.add("_blank");
            iconls.add(classForNonDir);

            let permstr = permsToString(file[1]);

            datals.push([
              permstr,
              + file[1].mtime,
              ((stat.isDirectory && stat.isDirectory()) ? 0 : stat.size),
              displayName
            ]);

            html += [
              `<tr><td><i class="icon ${iconClass}"></i></td>`,
              (hidePermissions ? "" : `<td class="perms">(${permstr})</td>`),
              `<td class="last-modified">${lastModifiedToString(file[1])}</td>`,
              `<td class="file-size">${sizeToString(file[1], humanReadable, si)}</td>`,
              `<td class="display-name"><a href="${href}"${isDir ? "" : 'target="_blank"'}>${displayName}</a></td>`,
              '</tr>',
            ].join('');
          };
          ...
          html = [
            ...
            `<style type="text/css">${styles.iconcompress(Array.from(iconls))}</style>`,
            `</head><body><h1>Index of ${he.encode(pathname)}</h1>`,
            `<table><thead><tr>`,
            `<th></th><th>属性</th><th class="paddingleft">修改日期</th><th class="paddingleft">大小</th>`,
            `</tr></thead><tbody>`, html, `</tbody></table>`,
          ].join('') + "\n";

          html += [
            `<p>Server running @ ${he.encode(req.headers.host || '')}</p>`,
            argv.detail ? `<p>${plugins.systeminfo}</p>` : "",
            `<script type="text/javascript">window.filedata=${JSON.stringify(datals)}</script>`,
            `<script type="text/javascript">${plugins.innerscript}</script>`,
            `<!-- (c) 2022 HowardZhangdqs -->`,
            `</body></html>`
          ].join("");
